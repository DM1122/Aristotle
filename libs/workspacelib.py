...
class Workspace:
    def __init__(self, paths, verbosity=0):
        ...
        print(
            '[workspace]: Instatiating workspace at "{}"'.format(os.getcwd())
        ) if self.verbosity >= 1 else False

        for path in self.paths:
            if not os.path.exists(path):
                print(
                    '[workspace]: Creating path "{}"'.format(path)
                ) if self.verbosity >= 2 else False
                os.mkdir(path)
            else:
                print(
                    '[workspace]: Path "{}" already exists'.format(path)
                ) if self.verbosity >= 2 else False
    ...
    def reset(self):
        ...
        for path in self.paths:
            try:
                shutil.rmtree(path)
                print(
                    '[workspace]: Reset "{}"'.format(path)
                ) if self.verbosity >= 2 else False
            except:
                print(
                    '[workspace]: Unable to reset "{}"'.format(path)
                ) if self.verbosity >= 2 else False
    ...
    def clear(self):
        ...
        for path in self.paths:
            try:
                shutil.rmtree(path)
                print(
                    '[workspace]: Removed "{}"'.format(path)
                ) if self.verbosity >= 2 else False
            except:
                print(
                    '[workspace]: Unable to remove "{}"'.format(path)
                ) if self.verbosity >= 2 else False
...
if __name__ == "__main__":
    ...
    ws = Workspace(
        paths=["folder_A", "folder_A/subfolder", "folder_B", "folder_C"], verbosity=2
    )
