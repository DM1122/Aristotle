...
def create():
    ...
    '''
    print('[databasemanager]: Instantiating database at "{}"'.format(database_path)) if verbosity >= 1 else False

    if os.path.isfile(database_path):
        print('[databasemanager]: Warning: database already exists') if verbosity >= 1 else False
        return
    ...
    print('[databasemanager]: Database created successfully') if verbosity >= 2 else False
...
def loadVideo(name):
    ...
    '''
    print('[databasemanager]: Loading video "{}..."'.format(name[0:16])) if verbosity >= 1 else False

    video_path = video_repo_path + '/' + name + '.mp4'      # will not work with all filetypes

    if not os.path.isfile(video_path):
        raise Exception('[databasemanager]: Video could not be found in repository. Consider downloading it first.')
